<div class="container mx-auto px-4 py-8">
    ...
    @auth()
        ...
        @else
            @if(!auth()->user()->hasAnyRole('admin','superadmin','items_management','cars_management') || !auth())
                <div class="flex justify-between items-center bg-yellow-100 text-yellow-800 p-3 rounded mb-4">
                    <h1>Please select role to show dashboard </h1>
                    <a href="{{ route('request') }}" class="mx-9">Select</a>
                </div>
            @elseif(auth()->user()->hasRole('cars_management'))
                <div class="flex justify-between items-center bg-yellow-100 text-yellow-800 p-3 rounded mb-4">
                    <h1>Cars dashboard will available soon </h1>
                </div>
            @endif
        @endif
    @endauth
    @guest()
        <div class="flex justify-between items-center bg-yellow-100 text-yellow-800 p-3 rounded mb-4">
            <h1>Register/login to show dashboard menu</h1>
        </div>
    @endguest
